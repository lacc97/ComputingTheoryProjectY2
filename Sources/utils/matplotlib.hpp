...
constexpr const char* QT5_BACKEND = "Qt5Agg";
constexpr const char* TK_BACKEND = "TkAgg";
...
template<class List>
PyObject* get_array(const List& v)
{
...
template<class List>
bool plot(const List &x, const List &y, const std::map<std::string, std::string>& keywords)
{
...
template<class ListX, class ListY>
bool scatter(const ListX& x,
             const ListY& y,
...
template<class ListX, class ListY>
bool plot(const ListX& x, const ListY& y, const std::string& s = "")
{
    assert(x.size() == y.size());
...
template<class ListX, class ListY>
bool named_plot(const std::string& name, const ListX& x, const ListY& y, const std::string& format = "")
{
    PyObject* kwargs = PyDict_New();
    PyDict_SetItemString(kwargs, "label", PyString_FromString(name.c_str()));
...
template<class List>
bool plot(const List& y, const std::string& format = "")
{
    List x(y.size());
